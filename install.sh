...
echo ""
sleep 1
yes | opkg install screen unzip redsocks
clear
echo ""
echo -e "\033[1;33mDownloading tools\033[1;37m"
echo ""
sleep 1
wget https://github.com/jakues/brainfuck-psiphon-installer/releases/download/0.1/brain.zip
echo ""
...
echo ""
sleep 1
cat > /usr/bin/brain << EOF
#brainfuck simple running script
#jakues
 start() {
        a=$(ls /usr/sbin | grep redsocks)
        b=$(ls /usr/sbin/ | grep screen)
        c="/root/brain/brainfuck-psiphon-pro-go"
        cd
        clear
        ...
        screen -d -m -L /root/brain/brainfuck-psiphon-pro-go
        sleep 3
 ...
 stop() {
        ...
        echo -e "Remove cache ..."
        sleep 1
        rm /root/screenlog.0 > /dev/null 2>&1
        rm /root/progo/redsocks.log > /dev/null 2>&1
